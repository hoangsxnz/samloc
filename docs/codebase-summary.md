...
| `room-code.ts` | 6-char room codes from 32-symbol alphabet, rejection-sampling guard, `generateRoomCode()`/`isRoomCode()` |

#### Durable Object (Room State Machine) (`src/room-do*.ts`)

| File | Responsibility |
|---|---|
| `room-do.ts` | RoomDO class: WebSocket hibernation API, snapshot routing, rate limiting (10 msg/s, burst 20) |
...
| `room-do-actions.ts` | `handleMessage()` dispatcher: join/ready/settings/start/nextHand/leave/play/pass/declareSam |
...
| `room-code.test.ts` | Length/alphabet/no-ambiguous-char, 10k-draw distribution |
...
| `lib/ws-client.svelte.ts` | WebSocket wrapper: `connected` state, `send()` with JSON stringify, auto-reconnect on close |
| `lib/room.svelte.ts` | `RoomStore` singleton: `view`, `ws`, `lastError`; methods `join()`/`ready()`/`settings()`/`start()`/`play()`/`pass()`/`declareSam()`/`nextHand()`/`leave()` |
| `lib/orientation.svelte.ts` | Landscape-lock detection and request on first pointer event |
| `lib/table-layout.ts` | Seat/card positioning math: `fanLayout()`, `tableScale()`, opponent positions |
...
- `playing-card.svelte` — lg/sm/xs card faces (56×80 / 44×62 / 24×32) + card-back with lattice
- `segmented-control.svelte` — 3-pill settings selector (turn seconds, player count, stake)
- `code-input.svelte` — 6-box room-code entry with auto-advance and paste-all
- `timer-ring.svelte` — SVG stroke-dasharray timer, size-parametric (`size`/`stroke`/`digits`); arc-only mode wraps the active opponent avatar; pulses red under 5s, `role="timer"`
...
- `centre-stack.svelte` — trick display (newest on top, fade-out on older combos)
- `opponent-seat.svelte` — 48px avatar, name, card-back count, active-turn ring, passed/disconnected states
- `me-chip.svelte` — bottom-left: avatar, name, current hand count, invalidReason or đền bài warning
...
| `table/table-logic.svelte.ts` | Turn logic: `combo`/`currentCombo`, `canPlay`, `denWarn`, event-tag queue (deduped, capped 3/seat), timer countdown |
...
Rules engine tests in `packages/rules/tests/` cover all combos, comparisons, deals, instant-wins, turn flow, settlement. Worker tests in `apps/worker/tests/` cover auth, room-codes, D1 room-session/hand-results queries. No browser-based tests (Vitest in Node environment; UI state verified by dev server).
